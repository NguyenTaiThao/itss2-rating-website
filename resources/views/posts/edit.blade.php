                                        ...
                                        <div class="col-md-7">
                                            <div class="form-group">
                                                ...
                                                <input value="{{ old('title', $post->title) }}" type="text"
                                                    ...
                                                    aria-describedby="titleHelper" placeholder="タイトルを書いてください">
                                                @if ($errors->has('title'))
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select
                                                    value="{{ old('product_category_id', $post->product_category_id) }}"
                                                    id="product_category_id" name="product_category_id"
                                                    class="form-control">
                                                    <option selected disabled>製品のカテゴリを選択してください</option>
                                                    @foreach ($productTypes as $productType)
                                                        <option value="{{ $productType->id }}"
                                                            {{ old('product_category_id', $post->product_category_id) == $productType->id ? 'selected' : '' }}>
                                                            {{ $productType->name }}</option>
                                                    @endforeach
                                                </select>
                                                @if ($errors->has('product_category_id'))
                                                    <div class="text-danger">
                                                        {{ $errors->first('product_category_id') }}</div>
                                                @endif
                                            </div>
                                        </div>
                                        <div class="col-md-5">
                                            ...
                                            <div id="divImageMediaPreview">
                                                <img src="{{ asset('storage/' . $post->img_url) }}" alt="post_image"
                                                    style="width:100%;">
                                            </div>
                                        </div>
                                    </div>
